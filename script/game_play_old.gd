...
func get_frame_clicked(pos: Vector2) -> int:
	...
	for i in patterns.size():
		if patterns[i][1].has_point(pos):
			retval = i
	printt("Get Frame Clicked: ", retval)
...
func set_frame_found(frame: int) -> void:
	...
	overlay.position = patterns[frame][0].position
	overlay.rect = patterns[frame][0]
	overlay.frame_index = frame
